...
export class ListProjectComponent implements OnInit {
  displayedColumns: string[] = ['no', 'name', 'startDate', 'endDate', 'completeDate', 'tienDo', 'status', 'action'];
        ...
        if(res?.length > 0){
          res.forEach((element: any, index: number) => {
            const project: Project = {
              ...
              tienDo: element.buocHienTai?.tenBuoc,
  ...
  openDialogCapNhatTrangThaiDuAn(projectId: string, statusInit: number){
    const dialogRef = this.dialog.open(DialogCapNhatTrangThaiDuAnComponent, {
      width: "35%",
      data: {id: projectId, statusInit: statusInit}
    });
    dialogRef.afterClosed().subscribe(result => {
      // console.log(`Dialog result: ${result}`);
      this.getAllProject();
