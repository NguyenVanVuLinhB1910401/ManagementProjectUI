...
@Component({
  selector: 'app-dialog-chuyen-buoc-tiep-theo',
  templateUrl: './dialog-chuyen-buoc-tiep-theo.component.html',
  styleUrls: ['./dialog-chuyen-buoc-tiep-theo.component.scss']
})
export class DialogChuyenBuocTiepTheoComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<DialogChuyenBuocTiepTheoComponent>,
  ...
  ngOnInit(): void {
    ...
    this.getBuocTiepTheo(this.data.buocTiepTheo);
  }

  getBuocTiepTheo(code: string){
  ...
  onSubmit(){
    if(this.createWorkForm.valid){
      ...
      this.workService.chuyenBuocTiepTheo(formData)
        .subscribe({
          next: (res: any) => {
            //console.log(res);
          ...
          error: (err) => {
            //console.log(err);
            //this.isClick = false;
            this.toast.error({detail: "Thông báo", summary: "Chuyển bước tiếp theo thất bại", duration: 5000, position: "topCenter"});
          }
        })
      }else{
        this.toast.warning({detail: "Thông báo", summary: "Ngày bắt đầu không được lớn hơn ngày kết thúc", duration: 5000, position: "topCenter"})
      }
    }else{
